...
type StegoSession struct {
	...
	// MaxUploadBytes is reported by the server at session creation. Kept so the direct-upload
	// cap can move server-side without an SDK release; 0 falls back to DefaultMaxUploadBytes.
...
func (s *Stego) CreateSession(sessionType string) (*StegoSession, error) {
	...
	data := resp.decodeMap()
	return &StegoSession{t: s.t, UploadUID: stringValue(data["upload_uid"]), SessionType: sessionType, MaxUploadBytes: intValue(data["max_upload_bytes"])}, nil
}
...
func (s *StegoSession) Upload(filePath string, step int) (string, error) {
	return s.t.routeUploadFile(filePath, s.UploadUID, step, s.MaxUploadBytes)
}
