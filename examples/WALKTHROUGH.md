...
running the snapWONDERS API against these inputs (the API output is the same whatever client calls it).
...
> `go run ./examples/analyse your-photo.jpg`.

---

## 1. Steganography — `examples/hideandreveal`
...
`client.Stego.Reveal(...)` with the same password extracts the secret back out. The stego output is a
**lossless** image (here WebP) — re-encoding it to a lossy format would destroy the hidden data.

---

## 2. Forensic analysis — `examples/analyse`

`client.Analyse.Run([]string{...})` grades each file, returns the forensic **verdicts**, and gives
...
- **`verdicts`** — the curated, user-safe forensic verdicts (mirrors what the web report shows, never
  ...
  never any underlying signature), and `findings[].{label,severity}`.

> **Added 2026-09-17:** `verdicts` can also carry `forensic` (ELA / noise-inconsistency tile counts)
> for any file, and — video only — `audio_splice` and `video_tamper` (inter-frame tamper, frame
> duplication, P-frame FFT, GPS timezone triangle). Not shown in the captured JSON above: `sample.png`
> is a still image, so the video-only fields never apply to it, and this walkthrough hasn't been
> re-run since they shipped. `examples/analyse/main.go` already prints all of them when present — run
> it against your own file to see the live shape. Full field reference: `docs/reference/API.md` (in
> the main snapWONDERS repo). In the SDK: `item.Verdicts`.
...
## 3. Conversion — `examples/convert`

`client.Convert.Run([]string{...}, snapwonders.WithOption("image_format", "webp"))` re-encodes media.
Input → output:
...
*Regenerate every artifact in `sample-output/` by running the three example programs with a valid
`SNAPWONDERS_API_KEY`.*
