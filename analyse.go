...
func (a *Analyse) CreateSession() (*AnalyseSession, error) {
	...
	data := resp.decodeMap()
	return &AnalyseSession{t: a.t, UploadUID: stringValue(data["upload_uid"]), MaxUploadBytes: intValue(data["max_upload_bytes"])}, nil
}
...
func (s *AnalyseSession) Upload(filePath string) (string, error) {
	return s.t.routeUploadFile(filePath, s.UploadUID, 1, s.MaxUploadBytes)
}
