...
// UploadFailure is implemented by every upload error, whichever method was used.
//
// There are two upload paths — a single-request direct POST and the resumable TUS protocol — and
// which one runs is chosen for the caller by file size. Matching on one concrete type would
// therefore mean matching one that only occurs for some file sizes, so both satisfy this:
//
...
// UploadError is returned when a direct (single-request) upload fails.
type UploadError struct{ Message string }

func (e *UploadError) Error() string { return e.Message }
func (*UploadError) snapwonders()    {}
func (*UploadError) uploadFailure()  {}
...
func (*TusUploadError) uploadFailure()  {}
